...
class TenantNotFoundError(DomainException):
    ...
    def __init__(self, tenant_id: str):
        super().__init__(message=f"Tenant '{tenant_id}' not found", code="TENANT_NOT_FOUND")
...
class ResolutionConflictError(DomainException):
    ...
    def __init__(self, entity_ids: list[str]):
        super().__init__(
            message=f"Cannot resolve entities: {entity_ids}", code="RESOLUTION_CONFLICT"
        )
...
class GraphIngestionError(DomainException):
    ...
    def __init__(self, detail: str):
        super().__init__(message=f"Graph ingestion failed: {detail}", code="GRAPH_INGESTION_ERROR")
