...
def clean_tables(migrated_postgres: str):
    """Truncates documents and semantic_chunks between tests."""
    with (
        psycopg.connect(migrated_postgres, autocommit=True) as conn,
        conn.cursor() as cur,
    ):
        cur.execute("TRUNCATE TABLE semantic_chunks, documents CASCADE;")
    yield
    with (
        psycopg.connect(migrated_postgres, autocommit=True) as conn,
        conn.cursor() as cur,
    ...
    def test_rls_enabled_and_forced_on_all_tenant_tables(self, migrated_postgres: str):
        ...
        with psycopg.connect(migrated_postgres) as conn, conn.cursor() as cur:
            cur.execute(
                ...
                WHERE n.nspname = 'public' AND c.relname IN ('documents', 'semantic_chunks');
                """
            )
            rows = {row[0]: (row[1], row[2]) for row in cur.fetchall()}

            assert "documents" in rows, "Table 'documents' not found in public schema"
            assert "semantic_chunks" in rows, "Table 'semantic_chunks' not found in public schema"

            for table_name, (rls_enabled, rls_forced) in rows.items():
            ...
            cur.execute(
                ...
                FROM pg_policies
                WHERE schemaname = 'public' AND tablename IN ('documents', 'semantic_chunks');
                """
            )
            policies = {row[0]: row[1] for row in cur.fetchall()}
            assert "documents" in policies, "Missing policy on 'documents'"
            assert "semantic_chunks" in policies, "Missing policy on 'semantic_chunks'"
            assert policies["documents"] == "tenant_isolation_policy"
            assert policies["semantic_chunks"] == "tenant_isolation_policy"
    ...
    def test_app_role_does_not_own_tables_and_lacks_bypassrls(self, migrated_postgres: str):
        ...
        with psycopg.connect(migrated_postgres) as conn, conn.cursor() as cur:
            ...
            cur.execute(
                ...
                WHERE c.relname IN ('documents', 'semantic_chunks');
                """
            )
